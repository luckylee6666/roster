...
最新版本：**v1.3.2** — 把每一家已登记的 CLI 按权限档逐档拉起来实测，七家全都查出真问题：Qwen 与 Gemini 的写入档从来没能用过、agy 每一轮都失败且文件写进自己的 scratch 目录、OpenCode 跑在 Roster 自己的目录里而不是所选项目。同时新增 Codex「请求批准」及配套审批提示。详情见[更新日志](CHANGELOG.md)。
