...
最新版本：**v1.2.14** — 修复 macOS 窗口与系统退出，强化数据迁移和备份，并提升终端及文件预览安全性。详情见[更新日志](CHANGELOG.md)。
...
- **会话恢复** — 记住上次终端标签布局（目录 + CLI），重开应用询问是否恢复；Claude 与 OpenCode 用 `--continue`，Codex 在原项目目录用 `resume --last` 接回最近对话
