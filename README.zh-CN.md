...
- **手机远程** — 打开「手机远程」扫码，手机就能选项目、看各家 CLI 合并的历史、开新对话或续接、按各家 CLI 自己的权限档位发指令、实时看回复、随时停止。电脑是唯一的执行者：手机请求与桌面输入框走同一条路，电脑上同步显示。面板可「保持连接」后台运行并显示标记，关闭即作废 PIN；终端镜像在单独的标签页。局域网 + PIN、明文 HTTP（只接受私网与 Tailscale 来源）；可选的安卓外壳 App 在 `mobile-android/`
...
- 发送前按所选 CLI 自己的**只读/计划策略**起步：每家第一档固定只读，空档位或不认识的档位也留在只读。发送按钮旁可切换权限档位，前端只传档位 ID，后端不认就拒，不会退化成“给个写入权限算了”；Roster 不会自动补额外批准，第三方插件、本机配置与联网行为仍由该 CLI 自己决定，而不是 Roster 提供的 OS 级沙箱。Command Code 是这条规则的极端情形：它的无头模式自己就拦下文件写入与 shell 命令，除非传内置的 `--yolo`，所以这家**默认只有只读档**。完全访问是显式选择项，且是按用户要求加的：在工作台把档位切到「完全访问」只影响那一轮，或者**右键项目卡片上的 `cmd` 色标**用 `--yolo` 打开那个终端。两处的默认项都是不带绕过参数的裸命令，Roster 不会自己加；以绕过参数启动的标签徽标转红色
...
底部抽屉式终端，点项目卡片底部「打开 CLI」或右下角悬浮按钮打开。
