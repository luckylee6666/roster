...
最新版本：**v1.4.1** — Grok 用量实时刷新、历史对话自动定位最新消息、会话内文件预览支持相对链接与行号定位，并防止多实例互相覆盖项目数据。详情见[更新日志](CHANGELOG.md)。
...
输入框左下角的 **＋** 菜单可添加图片或管理**常用指令**。保存了指令后才显示快捷选择；选中只填入草稿，不会自动发送。

### 项目共享记忆

会话模式只显示“项目记忆 · 自动”，正常使用无需操作。开启的项目会自动读取相关资料、在正常结束有实质进度的任务后记录简短摘录；切换助手时可继续使用。继续沿用项目的 Claude 记忆目录，不迁移或合并各家 CLI 的私有记忆。首次迁移保留旧开启列表；旧记录存在时，未在列表中的已有项目保持关闭。没有旧记录时仅复用已有 `.memory` 关联；之后新增项目默认开启。

普通会话在发送前附加索引、相关专题和最近两条自动进度，正文总量最多 12 KiB，单份最多 4 KiB；状态悬停可查看读取清单。自动进度保留最近 20 条、总量最多 48 KiB，存于 `inbox/.roster-recent.json`，带来源与时间，标注未经独立核实，不覆盖人工专题。普通 inbox 草稿不自动附加；CLI 失败/取消、闲聊及明显包含凭据的摘录不自动记录。自定义命令保留原生语义。CLI 安全模式和代码权限不变；关闭不会撤回已进入历史的内容。
...
- 中间显示自然语言对话，也会恢复经过校验的历史截图和项目内图片/视频链接；点击本地文本文件链接会直接在会话内打开只读预览，无需切换开发模式。预览内相对链接按当前文件目录解析；带行号链接显示源码并定位、高亮目标行。长对话使用独立的有界历史正文，不再受跨 CLI 交接摘要的短窗口限制。右侧可看分支、改动、最近提交、处理步骤和项目动态，也可打开文件夹、刷新现场，并对项目想法执行新增、完善、归档、删除或放入输入框。底部还能选择已有 Prompt 片段
