...
test('登记表覆盖现有 CLI，并按 id / 别名过滤', () => {
  assert.deepEqual(CLI_TOOL_IDS, ['claude', 'grok', 'codex', 'opencode', 'gemini', 'agy', 'qwen', 'mimo']);
...
test('最近用过的 CLI 排到最前，方向键在可见列表里循环', () => {
  ...
  assert.deepEqual(ordered.slice(1).map(tool => tool.id), ['claude', 'grok', 'opencode', 'gemini', 'agy', 'qwen', 'mimo']);
