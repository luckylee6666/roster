...
test('项目卡片底部列出本机已装 CLI，一点即开', () => {
  ...
  assert.match(main, /installedCliProbeRevision/);
  assert.match(main, /INSTALLED_CLI_MAX_RETRIES = 2/);
  assert.match(main, /!detected\.length && installedCliProbeRetries < INSTALLED_CLI_MAX_RETRIES/);
  ...
  assert.match(styles, /\.term-tab-tool\.tool-commandcode\b/);
