...
test('终端工具栏提供默认单窗到最多四窗格布局', () => {
  assert.match(html, /id="terminal-layout-btn"/);
  assert.match(html, /id="terminal-bodies"[^>]*data-layout="single"[^>]*data-arrangement="single"/);
  assert.deepEqual(
    [...html.matchAll(/<button[^>]*data-layout="(single|columns|rows|main|grid)"/g)].map(match => match[1]),
    ['single', 'columns', 'rows', 'main', 'grid'],
  );
  assert.match(html, /主从分屏/);
  assert.match(html, /空窗格会收拢成左右或主从/);
});
...
test('每个可见会话使用独立终端宿主并通过布局状态机选择窗格', () => {
  ...
  assert.match(main, /dataset\.arrangement = arrangement/);
  assert.match(styles, /\.term-body\.pane-visible\s*\{\s*display:\s*flex/);
  assert.match(styles, /\.terminal-bodies\[data-arrangement="main"\] \[data-pane-slot="0"\][^{]*\{[^}]*grid-row:\s*1\s*\/\s*span\s*2/);
  assert.match(styles, /\.terminal-bodies\[data-arrangement="grid"\][^{]*\{[^}]*grid-template-columns:[^}]*grid-template-rows:/);
});
...
test('移出分屏不会关闭 PTY，只有关闭标签才结束会话', () => {
  ...
  assert.doesNotMatch(main, /className = 'term-pane-empty'/);
});
...
test('分屏文件预览只覆盖当前活动窗格', () => {
  assert.match(styles, /\.terminal-bodies:not\(\[data-arrangement="single"\]\)\.preview-obscured\s*\{\s*visibility:\s*visible/);
  assert.match(styles, /\.terminal-bodies:not\(\[data-arrangement="single"\]\)\.preview-obscured \.term-body\.active\s*\{\s*visibility:\s*hidden/);
