...
test('两种工作台共用后端共享记忆偏好，开启的项目才挂载 .memory', () => {
  assert.match(main, /shared_memory_initialize/);
  assert.match(main, /shared_memory_state', \{ projectId: memoryProject.id \}/);
  assert.match(main, /if \(shared.enabled\)/);
  ...
  assert.match(main, /sharedMemoryController\?\.open\(project\)/);
