...
- **Multi AI CLI launch** — start **Claude / Grok / Codex / opencode / Gemini / agy / Qwen** in a project directory from the project card, with a tool badge on the tab. Opening a CLI focuses a running tab for that tool, otherwise resumes the latest on-disk session; a new session starts only when there is no history
