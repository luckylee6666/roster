...
**Fixed**
- A data-directory lifetime lock prevents concurrent Roster instances from overwriting each other's project lists. Exit older versions before launching this update. Media layout listeners are now registered once per element, and “Back to latest” restores delayed-media following. Nested preview links resolve relative to the open document; line links open source view and highlight the requested line.
- Local file links in conversation messages now open in a read-only preview inside the conversation workspace instead of being silently swallowed or forcing a switch to Developer mode. The backend only reads bounded regular text files under the current project or its matching Claude memory directory; web links still open in the system browser.
- Opening or automatically resuming a conversation now lands on its newest message for every CLI. The initial follow-to-bottom also waits for historical images and videos to finish affecting layout, while any deliberate upward scrolling immediately cancels that follow behavior.
...
**修复**
- 数据目录增加进程生命周期独占锁，阻止多份 Roster 用旧项目表互相覆盖；升级使用前应先退出旧版本。媒体布局监听改为每个元素只绑定一次，「回到最新」会恢复延迟媒体跟随；预览内链接按当前文档目录解析，带行号链接打开源码并定位、高亮目标行。
- 对话消息中的本地文件链接现在会直接在会话工作台内打开只读预览，不再被静默吞掉，也无需切换开发模式；后端只会有界读取当前项目或该项目对应 Claude 记忆目录中的普通文本文件，网页链接仍交给系统浏览器。
- 打开或自动续接任意 CLI 的历史对话时会直接定位到最新消息；历史图片和视频异步撑高布局后也会再次校准到底部，而用户主动向上浏览时会立即停止自动跟随。
